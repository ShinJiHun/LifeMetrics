    ...
    if (waypoints.length > 0) {
        ...
        for (let s = 1; s < segs; s++) {
            ...
            distances.forEach((d, i) => {
                if (Math.abs(d - target) < minD) {
                    minD = Math.abs(d - target);
                    idx = i;
                }
            });
            keyPoints.push({
...
function windEffect(bearing: number, windDir: number, windSpeed: number): {
    ...
    if (absDiff <= 45) return {
        label: "역풍",
        emoji: "💨",
        color: "#f87171",
        desc: `정면에서 ${windSpeed}km/h 맞바람 — 페이스 유지 주의`
    };
...
function ElevChart({
    ...
    const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
        ...
        let minD = Infinity, idx = 0;
        distances.forEach((d, i) => {
            if (Math.abs(d - targetDist) < minD) {
                minD = Math.abs(d - targetDist);
                idx = i;
            }
        });
            ...
            >
                ...
                <g transform={`translate(${pad.l},${pad.t})`}>
                    ...
                    {keyPoints.map((kp, i) => {
                        ...
                        return (
                            <g key={i}>
                                <line x1={x} y1={0} x2={x} y2={CH} stroke={c} strokeWidth="1" strokeDasharray="3,3"
                                      opacity="0.6"/>
                    ...
                    {hoveredPoint && (() => {
                        ...
                        return (
                            <g>
                                <line x1={x} y1={0} x2={x} y2={CH} stroke="#facc15" strokeWidth="1"
                                ...
                                <text x={labelX + 45} y={y - 10} fill="#facc15" fontSize="10" textAnchor="middle"
            ...
            {!w ? (
                ...
                <>
                    <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 14px", marginBottom: 8}}>
                        ...
                        <WCell icon="💧" label="강수확률" val={`${w.precip}%`}
                               accent={w.precip > 50 ? "#f87171" : undefined}/>
                            ...
                            <div style={{display: "flex", gap: 4}}>
                                {w.hourly.map((h) => (
                                    <div key={h.time} style={{
                                        ...
                                        <div style={{
                                            fontSize: 10,
                                            color: h.precip > 50 ? "#f87171" : "#475569"
                                        }}>{h.precip}%
                                        </div>
                                        <div
                                            style={{fontSize: 11, color: h.wind > 30 ? "#fbbf24" : "#64748b"}}>{h.wind}㎞
                                        </div>
                                        <div style={{
                                            fontSize: 14,
                                            transform: `rotate(${h.windDir}deg)`,
...
function WCell({icon, label, val, accent, full}: {
    icon: string;
    label: string;
    val: string;
    accent?: string;
            ...
            {step === "form" ? (
                <div style={S.formCard}>
                    ...
                    <div style={S.formGrid}>
                        ...
                        <FormField label="날짜">
                            <input style={S.input} type="date" value={eventDate}
                        ...
                        <FormField label="제한 시간 (시간)">
                            <input style={S.input} type="number" value={timeLimit}
                                   onChange={(e) => setTimeLimit(e.target.value)}/>
                        </FormField>
                        <FormField label="목표 평균 속도 (km/h)">
                            <input style={S.input} type="number" value={targetSpeed}
                  ...
                  ✓ {(track.totalDist / 1000).toFixed(1)}km · ↑{Math.round(track.gain)}m
                        ...
                        <FormField label="GPX 파일">
                            <label
                                style={{
                                    ...S.fileLabel,
                                    borderColor: track && gpxFile ? "#22c55e" : "#475569",
                                    color: track && gpxFile ? "#22c55e" : "#94a3b8"
                                }}
                            ...
                            >
                                <input type="file" accept=".gpx" style={{display: "none"}} onChange={(e) => {
                ...
                <div>
                    ...
                    <div style={{
                        ...
                        {track && (
                            <div
                                style={{borderTop: "1px solid #1e293b", background: "#0f172a", padding: "8px 8px 4px"}}>
                                <div style={{fontSize: 10, color: "#334155", paddingLeft: 8, marginBottom: 2}}>고도 프로파일
                                </div>
                                ...
                                return (
                                    <div key={cp.id} style={{display: "flex", alignItems: "flex-start"}}>
                                        <div style={{
                                            ...
                                            <div
                                                style={{width: 12, height: 12, borderRadius: "50%", background: dotC}}/>
                                            {idx < cps.length - 1 && <div style={{
                                                width: 2,
                                                flex: 1,
                                                background: "#334155",
                                                minHeight: 40,
                            ...
                            {idx === 0 ? "출발" : idx === cps.length - 1 ? "완주" : `CP${idx}`}
                                                    ...
                                                    <div>
                                                        ...
                                                        <div style={{fontSize: 12, color: "#64748b", marginTop: 2}}>
                                                            {cp.distance} km 지점
                                                            {segDist > 0 && <span
                                                                style={{color: "#3b82f6"}}> · +{segDist} km</span>}
                                                        </div>
                                                    </div>
                                                </div>
                                                <div style={{display: "flex", alignItems: "center", gap: 14}}>
                                            ...
                                            {isExp && (
                                                <div
                                                    style={{padding: "14px 18px 18px", borderTop: "1px solid #334155"}}>
                                                    {segDist > 0 && (
                                                        <div style={{display: "flex", gap: 10, marginBottom: 14}}>
                                                            {[
                                                                ...
                                                                {
                                                                    label: "예상 소요",
                                                                    val: `${Math.round(segDist / Number(targetSpeed) * 60)}분`
                                                                },
                                                                {
                                                                    label: "마감 여유",
                                                                    val: marginText(cp.deadline, cp.targetArrival),
                                                                    color: mc
                                                                },
